...
%extend BrelRelation_t {
  BrelRelation_t(DdManager* ddman, DdNode *relation, DdArray *inps, DdArray *ops) {
  ...
  ~BrelRelation_t() {
    if (self->inrel != NULL) { for (DdNode **tmp=self->inrel; tmp < self->inrel+self->numin; tmp++) Cudd_RecursiveDeref(self->ddman, *tmp); }
    BRELFREE(self->inrel);
    if (self->outrel != NULL and self->ddman) { for (DdNode **tmp=self->outrel; tmp < self->outrel+self->numout; tmp++) Cudd_RecursiveDeref(self->ddman, *tmp); }
    BRELFREE(self->outrel);
    if (self->ddman) Cudd_RecursiveDeref(self->ddman, self->relation);
    if (self->bddsol != NULL and self->ddman) { for (DdNode **tmp=self->bddsol; tmp < self->bddsol+self->numout; tmp++) Cudd_RecursiveDeref(self->ddman, *tmp); }
  ...
  DdArray* SolveRelation(BrelContext_t *ctxt = NULL) {
    ...
    for (i=1; i<self->numout; i++) {
      tmp = Cudd_bddAnd(self->ddman,self->outrel[i],opcube);
      Cudd_Ref(tmp);
      Cudd_RecursiveDeref(self->ddman,opcube);
      opcube = tmp;
    }
    self->bddsol = Brel_Solve(self->ddman,self->relation,opcube,(Brel_Context_t *) ctxt);
    // Done with opcube
    Cudd_RecursiveDeref(self->ddman, opcube);
    result = new DdArray(self->ddman,self->numout);
